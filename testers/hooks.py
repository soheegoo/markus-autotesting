...
# Helper functions
def upload_svn_file(api, group_repo_name, assignment_name, file_name, svn_user, svn_password, commit_message):
...
def after_all(api, assignment_id, group_id, group_repo_name):
    ...
    if os.path.isfile(feedback_name):
        with open(feedback_name) as feedback_open:
            ...
            # upload in svn repo
            upload_svn_file(api, group_repo_name, 'AX', feedback_name, 'svn_user', 'svn_password', 'Feedback file')
    # upload annotations
